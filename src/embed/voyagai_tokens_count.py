...
def chunk_code(
    code: str,
    path: str,
    MAX_CHARS=AVG_CHAR_IN_LINE * 10,  # 200 lines of code
    coalesce=AVG_CHAR_IN_LINE*2,
) -> list[str]:
...
def count_tokens(split_contents, file_path):
    vo = voyageai.Client()
    total_tokens = vo.count_tokens(split_contents, "voyage-code-3")
    print(f"本地方法计算的文件{file_path}的token数[{total_tokens}]")
    embeddings = vo.embed(split_contents, model="voyage-code-3", input_type="document")
    print(f"voyageai返回的token数[{embeddings.total_tokens}]")
    return total_tokens
        ...
        for file in files:
            ...
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    token_num += count_tokens(split_contents, file_path)
...
token_num = read_all_files("/Users/01428674/Downloads/test")
print(token_num)
